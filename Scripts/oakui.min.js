...
function createCore(type, size, ecallback, dcallback) {
    ...
    switch(type) {
        default:
            return new MIPSCore(size, ecallback, dcallback, addCycle, clearPipeline);
    }
}
...
function setConsoleMode(mode) {
    if (mode == 0) {
        $("#memory, #log, #pipeline").css("display", "none");
        $("#console").css("display", "block");
    }
    else if (mode == 1) {
        $("#console, #log, #pipeline").css("display", "none");
        $("#memory").css("display", "block");
    }
    else if (mode == 2) {
        $("#console, #memory, #pipeline").css("display", "none");
        $("#log").css("display", "block");
    }
    else {
        $("#console, #memory, #log").css("display", "none");
...
function addCycleHeader(num) {
    var node = document.createElement("th");
    var textnode = document.createTextNode(""+num);
    node.appendChild(textnode);
    document.getElementById("thead").appendChild(node);
}
...
var cycle = 0;
var line = 0;
function addCycle(instruction, cycleNames) {
    addCycleHeader(cycle);

    var body = document.getElementById("tbody");
    if (instruction != "") {
        var tr = document.createElement("tr");
        body.appendChild(tr);
        tr.appendChild(addData(line+1));
        tr.appendChild(addData(instruction));
        line++;
    }

    for (var i = 0; i < cycleNames.length; i++) {
        var row = body.children[line-i-1];
        
        if(row.children.length == 2)
            for (var j = 0; j < cycle; j++)
                row.appendChild(addBuffer())

        var node = addData(cycleNames[i]);
        if (cycleNames[i] == "STALL")
            node.className = "stall"
        else
            node.className = "cycle"
        row.appendChild(node);
    }
    
    for (var i = line-cycleNames.length-1; i >= 0; i--) {
        var row = body.children[i];
        var node = row.appendChild(addBuffer())
        row.appendChild(node);
...
(function() {
    ...
    function addTab(name, code, machinecode) {
        if (tabs.length != 0) {
            ...
            tabs[currentTab].pipeline = $("#pipeline").html();
    ...
    function switchToTab(num) {
        ...
        $("#pipeline").html(tabs[num].pipeline);
    ...
    function removeTab(id) {
        if (tabs.length == 1) {
            ...
            if (currentTab > id)
                ...
                $("#pipeline").html(tabs[currentTab].pipeline);
                displayMemory();
                setRegisterNames();
            }
        }
